...
public class Lookup {
	...
	private static String subdir="P9/messageList/9.txt";
	...
	public String lookupFile(String messageID, Integer TTL, String filename,String topology){
		...
		System.out.println("look up in P9");
		...
		if(TTL>0&&topology.equals("star")){
			...
			String temp=null;
			try {
				p90 = (P0Entry)Naming.lookup(serverP0);
				TTL=TTL-1;
				//TODO
				temp=p90.obtainNode(messageID,TTL,filename,topology);
				result=result+temp;
			...
			try {
				TTL=TTL-1;
				p98 = (P8Entry)Naming.lookup(serverP8);
				
				temp=p98.obtainNode(messageID,TTL,filename,topology);
				if(temp!=null)
					result=result+temp;
				P5Entry p95=(P5Entry)Naming.lookup(serverP5);
				temp=p95.obtainNode(messageID,TTL,filename,topology);
				if(temp!=null)
					result=result+temp;
			} catch (MalformedURLException e) {
				// TODO Auto-generated catch block
