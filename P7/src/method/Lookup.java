...
public class Lookup {
	...
	private static String subdir="P7/messageList/7.txt";
	...
	public String lookupFile(String messageID, Integer TTL, String filename,String topology){
		...
		System.out.println("look up in P7");
		...
		if(TTL>0&&topology.equals("star")){
			...
			P0Entry p70;
			try {
				p70 = (P0Entry)Naming.lookup(serverP0);
				TTL=TTL-1;
				temp=p70.obtainNode(messageID,TTL,filename,topology);
				if(temp!=null)
					result=result+temp;
			...
			try {
				TTL=TTL-1;
				p73 = (P3Entry)Naming.lookup(serverP3);
				P6Entry p76=(P6Entry)Naming.lookup(serverP6);
				temp=p73.obtainNode(messageID,TTL,filename,topology);
				if(temp!=null)
					result=result+temp;
				temp=p76.obtainNode(messageID,TTL,filename,topology);
				if(temp!=null)
					result=result+temp;
			} catch (MalformedURLException e) {
				// TODO Auto-generated catch block
